...
function parseSiteSettings(settings: unknown): SiteSettings {
...
sitesRouter.post("/", async (c) => {
  try {
    ...
    if (existingVerified) {
      return c.json(
        { error: "This domain has already been verified by another user." },
        { status: 409 },
      );
    }
...
sitesRouter.get("/", async (c) => {
  try {
    ...
    const sites = results.map((s: any) => ({
      ...
      settings: parseSiteSettings(s.settings),
...
sitesRouter.patch("/:siteId/settings", async (c) => {
  ...
  try {
    const body = await c.req.json();
    const { feedbackEmailNotifications } = body;

    if (typeof feedbackEmailNotifications !== "boolean") {
      return c.json(
        { error: "feedbackEmailNotifications must be a boolean" },
        400,
      );
    }

    const site = await c.env.DB.prepare(
      "SELECT settings FROM sites WHERE id = ?",
    )
      .bind(siteId)
    ...
    const settings = {
      ...
      feedbackEmailNotifications,
    };

    await c.env.DB.prepare(
      "UPDATE sites SET settings = ?, updated_at = datetime('now') WHERE id = ?",
    )
      .bind(JSON.stringify(settings), siteId)
      .run();

    return c.json({ success: true, settings });
  } catch (error) {
    console.error("[Worker] Update site settings error:", error);
  ...
  try {
    ...
    if (!site.verification_token) {
      ...
      return c.json({
        ...
        verified: false,
        error:
          "Verification token was missing. A new one has been generated. Please add it to your DNS records and try again.",
        token,
      });
    }

    // Verify DNS
    const isVerified = await verifyDnsToken(
      site.domain,
    ...
    if (isVerified) {
      ...
      return c.json({
        ...
        verified: false,
        error:
          "Verification token not found in DNS TXT records. Please check your DNS settings and try again. Note that DNS changes can take some time to propagate.",
      });
    }
  } catch (error) {
