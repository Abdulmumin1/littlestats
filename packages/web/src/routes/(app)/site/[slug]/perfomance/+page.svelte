<script>
	...
	let loadTimeFilter = $state('all');
	let memoryFilter = $state('all');
	let currentSort = $state({ key: 'url', asc: true });
...
<LoadingBoundary {loading} label="Loading performance analytics">
	{#snippet fallback()}
		<SitePageShell>
			<div class="space-y-6">
				<div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-3">
					{#each Array(3) as _, index (index)}
						<div class="space-y-3 border border-stone-100 bg-stone-50 p-4 py-8 text-center dark:border-stone-700 dark:bg-stone-800">
							<div class="mx-auto h-4 w-28 animate-pulse bg-stone-100 dark:bg-stone-700/80"></div>
							<div class="mx-auto h-7 w-24 animate-pulse bg-stone-100 dark:bg-stone-700/90"></div>
						</div>
					{/each}
				</div>

				<div class="space-y-4 border border-stone-100 bg-white/60 p-4 dark:border-stone-700 dark:bg-stone-800">
					<div class="flex items-center justify-between gap-4">
						<div class="h-6 w-44 animate-pulse bg-stone-100 dark:bg-stone-700/80"></div>
						<div class="h-8 w-8 animate-pulse bg-stone-100 dark:bg-stone-700/80"></div>
					</div>
					<div class="flex justify-between gap-4 border-b border-stone-200 py-2 dark:border-stone-700">
						<div class="h-3 w-10 animate-pulse bg-stone-100 dark:bg-stone-700/80"></div>
						<div class="flex gap-3">
							<div class="h-4 w-20 animate-pulse bg-stone-100 dark:bg-stone-700/80"></div>
							<div class="h-4 w-16 animate-pulse bg-stone-100 dark:bg-stone-700/80"></div>
						</div>
					</div>
					<div class="space-y-2">
						{#each Array(7) as _, index (index)}
							<div class="space-y-2 border border-stone-100 bg-stone-50 p-2 dark:border-stone-800 dark:bg-stone-700/40">
								<div class="h-4 w-1/2 animate-pulse bg-stone-100 dark:bg-stone-700/80"></div>
								<div class="h-6 w-4/5 animate-pulse bg-stone-100 dark:bg-stone-700/80"></div>
								<div class="h-6 w-2/3 animate-pulse bg-stone-100 dark:bg-stone-700/80"></div>
							</div>
						{/each}
					</div>
				</div>
