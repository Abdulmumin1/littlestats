<script>
	...
	async function fetchFromDefaultDates() {
		...
		// Use v2 API instead of form actions
		try {
			// Construct a date range for the last 30 days (default)
			...
			// Fetch summary stats
			const stats = await api.getStatsSummary(domain.id, range);
			const timeSeries = await api.getTimeSeries(domain.id, range, 'day');
			const events = await api.getEventsList(domain.id, range);
			
			// Transform to match expected format
			views = {
				viewCount: stats.views || 0,
				graph: timeSeries.data.map(d => d.views) || [],
				dau: calculateDailyActiveUsers(events.events) || {},
				wau: calculateWeeklyActiveUsers(events.events) || {}
			};
			
			// For now, populate DAU/WAU with placeholder or calculate if possible
			// This is a simplified version until the API supports these specific metrics directly
			
		} catch (error) {
			console.error('Error fetching site card data:', error);
