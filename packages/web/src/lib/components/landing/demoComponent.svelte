<script>
    ...
    // Mock Data Source
    let mockEvents = $derived(events);

	// Drilldown filters (Local state for demo)
	let drilldown = $state({
		pagePath: null,
		referrer: null,
		country: null
	...
	async function fetchModalData() {
		...
		try {
			...
			if (activeModal === 'pages') {
				const res = getMockPages(mockEvents, filter);
                // Simple client-side search for mock
                modalItems = modalSearch.trim() 
                    ? res.filter(p => p.path.toLowerCase().includes(modalSearch.toLowerCase())) 
                    : res;
			} else if (activeModal === 'referrers') {
				const res = getMockReferrers(mockEvents, filter);
				modalItems = modalSearch.trim() 
                    ? res.filter(r => r.referrer.toLowerCase().includes(modalSearch.toLowerCase())) 
                    : res;
			} else if (activeModal === 'countries') {
				const res = getMockCountries(mockEvents, filter);
				modalItems = modalSearch.trim() 
                    ? res.filter(c => c.country.toLowerCase().includes(modalSearch.toLowerCase())) 
                    : res;
			}
		} catch (err) {
	...
	function toggleFilter(kind, value) {
        if (kind === 'page') {
            drilldown.pagePath = drilldown.pagePath === value ? null : value;
        } else if (kind === 'referrer') {
            drilldown.referrer = drilldown.referrer === value ? null : value;
        } else if (kind === 'country') {
            drilldown.country = drilldown.country === value ? null : value;
        }
	}
	...
	async function fetchDashboardData() {
		...
		try {
            ...
            // Generate/Refresh events if needed based on range or just use existing
            // For demo, we might want to ensure we have events covering the range
            if (mockEvents.length === 0) {
                 mockEvents = generateRandomEvents(2000);
            }

			const statsData = getMockStatsSummary(mockEvents, filter);
			const seriesData = getMockTimeSeries(mockEvents, filter, timeSeriesGranularity);
			const refsData = getMockReferrers(mockEvents, filter).slice(0, 5);
			const pagesData = getMockPages(mockEvents, filter).slice(0, 5);
			const countriesData = getMockCountries(mockEvents, filter).slice(0, 5);
			const devicesData = getMockDevices(mockEvents, filter);
...
<div class="p-6 text-stone-900 dark:text-stone-100 space-y-8 max-w-7xl mx-auto rounded-none">
	{#if loading && !stats}
		<LoadingState />
	{/if}
	...
	{#if stats}
		<div class="flex flex-wrap gap-2">
			...
			{#if drilldown.pagePath || drilldown.referrer || drilldown.country}
				<button onclick={() => { drilldown.pagePath = null; drilldown.referrer = null; drilldown.country = null; }} class="px-3 py-1 text-[10px] font-black uppercase tracking-widest border border-stone-200 dark:border-stone-800 text-stone-500 dark:text-stone-400 rounded-none">
					Clear
				</button>
			{/if}
		</div>
