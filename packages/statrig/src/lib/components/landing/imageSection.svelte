	...
	<div class="relative mx-auto max-w-4xl">
		<div class="relative flex flex-wrap justify-center gap-2 md:gap-4" bind:this={tabsContainer}>
			{#each features as feature}
				...
				<button
					data-tab={feature.id}
					class="group relative z-10 flex items-center gap-2 border border-black/10 px-4 py-2 text-sm font-medium transition-all duration-200 md:text-base
						{activeTab === feature.id
						? `bg-white text-${$color}-700 border-${$color}-700 dark:bg-stone-900 dark:text-${$color}-300 dark:border-${$color}-300`
		...
		{#each features as feature}
			...
			>
				<div class="space-y-8">
					...
					<!-- Demo Component -->
					<div
						class="relative border border-black/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-stone-900 dark:text-white"
					>
