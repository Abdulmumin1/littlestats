...
<div class="relative">
	<button
		...
		class="flex items-center gap-1 border border-black/10 bg-white dark:bg-stone-800 dark:text-white {hide
			? 'p-1'
			: ' px-2 py-1'}"
		>{#if !hide}
			Change Theme
	...
	{#if pickerOpen}
		<div
			...
			transition:fly={{ y: 10 }}
			class="glass-effect absolute z-100 mt-1 grid w-52.5 translate-x-[-50%] grid-cols-6 gap-4 border border-black/10 bg-white/80 p-4 dark:border-white/10 dark:bg-stone-800/80 {bottom
				? 'bottom-12 left-[63%]'
				: 'left-[50%]'}"
		>
