	...
	<form use:enhance={handleUpdate} action="?/updateUser" method="POST" class="space-y-6">
		...
		<div class="flex justify-end">
			<button
				type="submit"
				disabled={loading}
				class="flex items-center justify-center gap-1 rounded-2xl border-b-4 border border-{$color}-800 text-white bg-{$color}-600 dark:bg-{$color}-700 px-6 py-2 font-bold text-gray-100 hover:bg-{$color}-600 dark:bg-{$color}-700"
			>
				Save Changes
				{#if loading}
