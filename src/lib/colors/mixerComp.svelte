...
<div class="relative">
	...
	{#if pickerOpen}
		<div
			use:clickOutside
			on:click_outside={closePicker}
			transition:fly={{ y: 10 }}
			class="glass-effect absolute z-50 border-{$color}-300 mt-1 grid w-[210px] grid-cols-6 gap-4 border bg-{$color}-100 left-[50%] translate-x-[-50%] rounded-xl p-4"
		>
