...
<div class="min-h-screen p-4 text-white">
	<h1 class="mb-4 px-2 pt-4 text-2xl font-bold text-gray-100 md:text-3xl">Logs</h1>
	...
	<div class="mt-5 flex flex-col gap-8">
		...
		<div class="flex flex-col gap-6 md:flex-row-reverse">
				...
				<div class="flex flex-wrap gap-2">
					{#each logBuckets as [level, logs], index}
						...
						<button
							...
							class="flex flex-1 flex-col rounded-lg bg-{$color}-100/50 p-4 transition-colors dark:bg-stone-800/50  {activeLogLevel == index? `border-2 border-${levelColors[logBuckets[activeLogLevel][0]]}-500`:''}"
						>
							...
							<span class="rounded-full bg-black/20 px-1 py-0 text-2xl font-bold">
								{logs.length}
							</span>
						</button>
						<!-- {/if} -->
			...
			<div class="flex-1">
				<h2 class="mb-4 text-xl font-bold">Statistics</h2>
