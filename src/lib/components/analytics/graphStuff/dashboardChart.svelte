<script>
	...
	let unsubscribeColor = color.subscribe((c) => {
		if (chart && chartData) {
			// console.log(c);
			try {
				chart.data.labels = chartData.map((d) => formatDate(d.myX));
	...
	const MountChart = () => {
		...
		chart = new Chart(ctx, {
			type: chartType,
			data: {
				labels: chartData.map((d) => formatDate(d.myX)),
				datasets: [
					{
	...
	let c = $derived(chartData.map((d) => formatDate(d.myX)));
	$effect(() => {
		if (chart && chartData) {
			chart.data.labels = chartData.map((d) => formatDate(d.myX));
			chart.data.datasets[0].data = chartData.map((d) => d.myY);
...
<canvas bind:this={chartCanvas} class="h-full w-full max-w-full pr-2"></canvas>
